...
@pytest.fixture
def task_repository(neo4j_container: Neo4jContainer) -> Generator[Neo4jTaskRepository, None, None]:
...
def test_create_task(task_repository: Neo4jTaskRepository):
    ...
    retrieved = task_repository.get(str(task.id))
...
def test_get_nonexistent_task_returns_none(task_repository: Neo4jTaskRepository):
    """get() must return None when the Task ID does not exist."""
    result = task_repository.get(str(uuid4()))
