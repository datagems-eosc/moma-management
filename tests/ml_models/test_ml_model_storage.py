...
async def ml_model_repository(neo4j_container: Neo4jContainer) -> AsyncGenerator[Neo4jMlModelRepository, None]:
...
@pytest.mark.asyncio
async def test_create_ml_model(ml_model_repository: Neo4jMlModelRepository):
    ...
    created = await ml_model_repository.create(node)
    assert str(created.id) == str(node.id)

    retrieved = await ml_model_repository.get(str(node.id))
...
@pytest.mark.asyncio
async def test_get_nonexistent_ml_model_returns_none(ml_model_repository: Neo4jMlModelRepository):
    """get() must return None when the ML_Model ID does not exist."""
    result = await ml_model_repository.get(str(uuid4()))
    assert result is None


@pytest.mark.asyncio
async def test_update_ml_model(ml_model_repository: Neo4jMlModelRepository):
    ...
    await ml_model_repository.create(node)
    ...
    result = await ml_model_repository.update(updated_node)
...
@pytest.mark.asyncio
async def test_update_nonexistent_ml_model_returns_zero(ml_model_repository: Neo4jMlModelRepository):
    ...
    result = await ml_model_repository.update(node)
    assert result["updated"] == 0


@pytest.mark.asyncio
async def test_delete_ml_model(ml_model_repository: Neo4jMlModelRepository):
    ...
    await ml_model_repository.create(node)
    deleted = await ml_model_repository.delete(str(node.id))
    assert deleted == 1

    assert await ml_model_repository.get(str(node.id)) is None


@pytest.mark.asyncio
async def test_delete_nonexistent_returns_zero(ml_model_repository: Neo4jMlModelRepository):
    """delete() must return 0 when the node does not exist."""
    result = await ml_model_repository.delete(str(uuid4()))
    assert result == 0


@pytest.mark.asyncio
async def test_list_ml_models(ml_model_repository: Neo4jMlModelRepository):
    ...
    await ml_model_repository.create(n1)
    await ml_model_repository.create(n2)

    models = await ml_model_repository.list()
...
@pytest.mark.asyncio
async def test_has_referencing_aps_false_when_no_ap(ml_model_repository: Neo4jMlModelRepository):
    ...
    await ml_model_repository.create(node)
    assert await ml_model_repository.has_referencing_aps(str(node.id)) is False


@pytest.mark.asyncio
async def test_has_referencing_aps_true_when_ap_exists(neo4j_container: Neo4jContainer):
    ...
    async with driver.session() as session:
        ...
        await ap_repo.create(ap)

        assert await ml_repo.has_referencing_aps(ml_id) is True
...
@pytest.mark.asyncio
async def test_ap_retrieval_excludes_ml_model_nodes(neo4j_container: Neo4jContainer):
    ...
    async with driver.session() as session:
        ...
        retrieved = await ap_repo.get(ap_id)
        assert retrieved is not None
