...
The MoMa Management API is responsible for ingesting, storing, and exposing **Metadata Object Model (MoMa)** property graphs in [Neo4j](https://neo4j.com/). It manages **datasets**, **analytical patterns (APs)**, **dataset relationships**, **tasks**, and individual graph **nodes**. Dataset profiles in [Croissant](https://docs.mlcommons.org/croissant/docs/) format are converted to PG-JSON according to the MoMa schema and persisted. Analytical patterns describe data-processing workflows over datasets and support natural-language semantic search. Dataset relationships ("dataset linking") record that two datasets were found similar by an external comparison pipeline.
